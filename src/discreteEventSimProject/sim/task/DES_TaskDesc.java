package discreteEventSimProject.sim.task;
 ...
 * @author john
 */
public class DES_TaskDesc {
    /**
     * these vals will not change until map rebuilt/reset idx of the task described by this taskDesc
     */
    ...
    /**
     * size of team required to complete this task at optimal per-uav speed 
     * (more will speed up completion on diminishing returns, less will slow down completion)
     */
    public final int optUAVTeamSize;
    /**
     * time to complete resource task for team of size optUAVTeamSize - time is in milliseconds; 
     */
    public final float timeForOptToCmp;
    /**
     * minTimeToComplete is min time to complete task (hard threshold) 
     */
    public final float minTimeToComplete;
    /**
     * scaling factor for std for completion time - might need to tweak this depending on random completion time results. this is so STD is dependent on team size, with smaller teams having larger stds than larger ones
     */
    ...
    public DES_TaskDesc(int _idx, myPointf _tl, boolean _isGrp, 
    ...
    /**
     * calculate diminishing returns function, based on passed power (n) and value (x). allow sweep through powers for reporting
     * @param x
     * @param n
     * @return
     */
    ...
    /**
     * return an array of doubles that shows the effects of sweeping p (diminishing returns formula) 
     * and n(# of uavs in team) for this task. minP must be greater than 0, max P can be anything
     * @param minSize
     * @param maxSize
     * @param minP
     * @param maxP
    ...
    public String[] getTaskPerfForNDataCSV(int minSz, int maxSz, double minP, double maxP, double pwrIncr) {
        ...
        //add header
        tmpRes.add(DES_TaskDesc.getTaskPerfForNCSVHdr(minSz,maxSz, minP, maxP, pwrIncr));
