...
public abstract class myDispWindow {
	...
	public static final float xOff = 20 , yOff = 18.0f * (UAV_DESSim.txtSz/12.0f), btnLblYOff = 2 * yOff, rowStYOff = yOff*.15f;
	...
	public static final int[] editCrcFillClrs = new int[] {UAV_DESSim.gui_FaintMagenta, UAV_DESSim.gui_FaintGreen};			
	...
	public myDispWindow(UAV_DESSim _p, String _n, int _flagIdx, int[] fc,  int[] sc, float[] rd, float[] rdClosed, String _winTxt, boolean _canDrawTraj) {
		pa=_p;
		ID = winCnt++;
		name = _n;
		pFlagIdx = _flagIdx;
		...
		winText = _winTxt;
		trajFillClrCnst = UAV_DESSim.gui_Black;		//override this in the ctor of the instancing window class
		trajStrkClrCnst = UAV_DESSim.gui_Black;
...
//		tmp = new myGUIObj(pa, this,i, guiObjName, xyDims[0], xyDims[1], xyDims[2], xyDims[3], guiMinMaxModVals, guiStVal, guiBoolVals, off);		
	...
	protected void showKeyPt(myPoint a, String s, float rad){	pa.show(a,rad, s, new myVector(10,-5,0), UAV_DESSim.gui_Cyan, getFlags(trajPointsAreFlat));	}	
	...
	protected void dispBttnAtLoc(String txt, float[] loc, int[] clrAra){
		pa.setFill(clrAra);
		pa.setColorValStroke(UAV_DESSim.gui_Black);
		pa.rect(loc);		
		pa.setColorValFill(UAV_DESSim.gui_Black);
		//pa.translate(-xOff*.5f,-yOff*.5f);
	...
	protected void drawMouseBox(){
		if( getFlags(showIDX)){
		    pa.setColorValFill(UAV_DESSim.gui_LightGreen );
			pa.rect(closeBox);
			pa.setFill(strkClr);
			pa.text("Close" , closeBox[0]-35, closeBox[1]+10);
		} else {
		    pa.setColorValFill(UAV_DESSim.gui_DarkRed);
			pa.rect(closeBox);
			pa.setFill(strkClr);
...
//		pa.popStyle();pa.popMatrix();		
	...
	protected void drawNotifications(){		
		...
		dispMenuTxtLat("Drawing curve", pa.getClr((getFlags(drawingTraj) ? UAV_DESSim.gui_Green : UAV_DESSim.gui_Red)), true);
		//pa.show(new myPoint(0,0,0),4, "Drawing curve",new myVector(10,15,0),(getFlags(this.drawingTraj) ? pa.gui_Green : pa.gui_Red));
		//pa.translate(0,-30);
		dispMenuTxtLat("Editing curve", pa.getClr((getFlags(editingTraj) ? UAV_DESSim.gui_Green : UAV_DESSim.gui_Red)), true);
...
class mySideBarMenu extends myDispWindow{
	...
	public int[][] guiBtnSt = new int[][]{
		...
		new int[]{1,0},//,0,0,0},                   					//debug - momentary
	...
	public mySideBarMenu(UAV_DESSim _p, String _n, int _flagIdx, int[] fc, int[] sc, float[] rd, float[] rdClosed, String _winTxt, boolean _canDrawTraj) {
		super(_p, _n, _flagIdx, fc, sc,  rd, rdClosed, _winTxt, _canDrawTraj);
		guiBtnStFillClr = new int[]{		//button colors based on state
				UAV_DESSim.gui_White,								//disabled color for buttons
				UAV_DESSim.gui_LightGray,								//not clicked button color
				UAV_DESSim.gui_LightBlue,									//clicked button color
			};
		guiBtnStTxtClr = new int[]{			//text color for buttons
				UAV_DESSim.gui_LightGray,									//disabled color for buttons
				UAV_DESSim.gui_Black,									//not clicked button color
				UAV_DESSim.gui_Black,									//clicked button color
			};			
...
class myDrawnSmplTraj {
	...
	public myDrawnSmplTraj(UAV_DESSim _p, myDispWindow _win,float _topOffy, int _fillClrCnst, int _strkClrCnst, boolean _flat, boolean _smCntl){
		pa = _p;
...
class myScrollBars{
	...
	public myScrollBars(UAV_DESSim _pa,myDispWindow _win){
		pa = _pa;
		win = _win;
		ID = scrBarCnt++;
		setSize();
